# PocketRail

A Next.js wallet-wrapper app for the Web3NZ hackathon idea: users register or log in, choose whether they already have a wallet, link an external wallet or create an embedded wallet, keep their session across refreshes, view assets, and send funds.

## What works

- Email/password registration and login.
- Persistent HTTP-only session cookie, so refresh does not log the user out.
- SQLite database in `data/app.sqlite` for users, sessions, and transfers.
- Registration flow asks whether the user already has a wallet.
- External wallet linking through `window.ethereum` and optional Privy wallet UI.
- Embedded wallet creation for local/demo use when the user does not attach one.
- ENS reverse lookup for linked/generated wallet addresses.
- ENS recipient resolution in the send form, so `name.eth` can be used instead of a raw address.
- Real card/bank entry points through hosted Banxa sandbox on-ramp and off-ramp checkout.
- Hackathon test NZD balance with top-up/withdraw controls and conversion estimates.
- Send flow starts from NZD, estimates conversion, and supports common stablecoins such as USDC, USDT, DAI, PYUSD, FDUSD, TUSD, and BUSD.
- Live NZD price estimates through CoinGecko.
- Balance lookup for ETH and common stablecoins on Sepolia and Ethereum.
- Send flow for embedded wallets through the server wallet, and external wallets through the browser signer.
- Password-confirmed private-key export for app-generated wallets.
- Incoming/outgoing transaction history from Blockscout, plus locally recorded PocketRail sends.

## Run it

```bash
npm install
npm run dev
```

The current dev server is running at:

```text
http://localhost:3001
```

Port `3000` was already occupied on this machine, so Next selected `3001`.

## Environment

Copy `.env.example` to `.env` and set these before using anything beyond local demos:

```bash
SESSION_SECRET="a-long-random-secret"
WALLET_ENCRYPTION_KEY="a-long-random-wallet-encryption-secret"
NEXT_PUBLIC_PRIVY_APP_ID="your-privy-app-id"
BANXA_PARTNER_NAME="your-banxa-partner-name"
BANXA_ENV="sandbox"
SEPOLIA_RPC_URL="optional-custom-rpc"
ETHEREUM_RPC_URL="optional-custom-rpc"
```

Privy is wired through `@privy-io/react-auth`. When `NEXT_PUBLIC_PRIVY_APP_ID` is present, the Privy button appears and uses Privy's `connectOrCreateWallet` flow. Without it, the app still works locally by creating an encrypted demo EVM wallet on the server.

ENS names are resolved through Ethereum mainnet reverse lookup. If the attached wallet has a valid reverse ENS record, the dashboard shows the ENS name next to the wallet and in account details.

Transaction history uses Blockscout public APIs:

- Sepolia: `https://eth-sepolia.blockscout.com/api/v2`
- Ethereum: `https://eth.blockscout.com/api/v2`

Private-key export only works for wallets generated by PocketRail. Linked wallets and Privy-managed wallets do not expose private keys to this app.

Card top-ups and bank withdrawals are launched through Banxa's hosted sandbox checkout so PocketRail never collects or stores raw card/bank details. Use `BANXA_ENV=production` only after your Banxa partner account, domain, payment methods, KYC/AML settings, and supported corridors are approved.

For hackathon demos, the dashboard also includes a local test NZD balance. It is intentionally not real money: it lets judges try the account balance, top-up, withdrawal, and conversion UX immediately while keeping the architecture ready for Banxa or another regulated provider later.

Banxa sandbox notes:

- You need a Banxa partner name from Banxa onboarding.
- Sandbox checkout opens `https://{BANXA_PARTNER_NAME}.banxa-sandbox.com`.
- Production checkout opens `https://{BANXA_PARTNER_NAME}.banxa.com`.
- Buy flow passes `coinType=ETH`, `fiatType=NZD`, `fiatAmount=50`, `blockchain=ETH`, and the user wallet address.
- Sell flow passes `orderType=sell`, `coinType=ETH`, `fiatType=NZD`, `blockchain=ETH`, and the user wallet address.

## Production notes

For a real-money deployment, prefer Privy embedded wallets over server-held private keys, add email verification, require signed wallet-link verification, and use a production database such as Postgres. The server-created wallet path is useful for hackathon demos and local testing, but it is not the custody model you would ship for consumer funds.
